/**
 * @file
 * @brief BACnet server + client (Jeedom) avec socket JSON
 * Serveur BACnet complet + fonctionnalités client pour discovery/read/write
...
/* ============================================================================
 * STRUCTURES ET VARIABLES CLIENT (pour discovery/read/write sur réseau)
 * ============================================================================ */
...
static DISCOVERED_DEVICE device_list[MAX_DISCOVERED_DEVICES];
...
/* Forward declarations des handlers client */
static void client_i_am_handler(uint8_t *service_request, uint16_t service_len,
    BACNET_ADDRESS *src);
static void client_read_property_ack_handler(uint8_t *service_request,
    uint16_t service_len, BACNET_ADDRESS *src, BACNET_CONFIRMED_SERVICE_ACK_DATA *service_data);
static void client_read_property_multiple_ack_handler(uint8_t *service_request,
    uint16_t service_len, BACNET_ADDRESS *src, BACNET_CONFIRMED_SERVICE_ACK_DATA *service_data);
static void client_read_range_ack_handler(uint8_t *service_request,
    uint16_t service_len, BACNET_ADDRESS *src, BACNET_CONFIRMED_SERVICE_ACK_DATA *service_data);
static void client_write_property_ack_handler(BACNET_ADDRESS *src,
    uint8_t invoke_id);
static void client_cov_notification_handler(uint8_t *service_request,
    uint16_t service_len, BACNET_ADDRESS *src);
static void client_error_handler(BACNET_ADDRESS *src, uint8_t invoke_id,
    BACNET_ERROR_CLASS error_class, BACNET_ERROR_CODE error_code);
static void client_abort_handler(BACNET_ADDRESS *src, uint8_t invoke_id,
...
static int handle_client_readprop(json_t *root);
static int handle_client_writeprop(json_t *root);
                           ...
                           bool enable)
{
    ...
    if (Trend_Log_Configure_Direct(instance, source_type, source_instance, 
        ...
        if (Trendlog_Test_Source_Read(instance)) {
            printf("  ✓ Trendlog %u: Source read test passed (5 bytes)\n", instance);
...
static int apply_config_from_json(const char *json_text, bool full_reset)
{
    ...
    root = json_loads(json_text, 0, &jerr);
        ...
        if (strcmp(typ, "analog-input") == 0) {
         ...
         if (name && name[0] != '\0') {
            ...
            if (json_is_array(weekly_schedule)) {
                ...
                printf("  Configuring weekly schedule (%u days provided)...\n", (unsigned int)num_days);
                
                if (num_days < 7) {
                    printf("  WARNING: Only %u day(s) provided, expecting 7 days!\n", (unsigned int)num_days);
                }
...
static int handle_socket_line(const char *line)
{
    ...
    /* Try to parse as JSON first (for client commands) */
    if (line[0] == '{') {
        root = json_loads(line, 0, &error);
        if (root) {
            /* Try client command handlers */
            if (handle_client_whois(root) == 0) {
                json_decref(root);
                return 0;
            }
            if (handle_client_devicelist(root) == 0) {
                json_decref(root);
                return 0;
            }
            if (handle_client_objectlist(root) == 0) {
                json_decref(root);
                return 0;
            }
            if (handle_client_readprop(root) == 0) {
                json_decref(root);
                return 0;
            }
            if (handle_client_writeprop(root) == 0) {
                json_decref(root);
                return 0;
            }
            /* Add more client handlers here as needed */
            
            json_decref(root);
            /* If no client handler matched, fall through to legacy commands */
        }
    }
    ...
    if (strncmp(line, "CFGJSON ", 8) == 0) {
        ...
        fflush(stdout);
        int rc = apply_config_from_json(json, false);
    ...
    if (strncmp(line, "CFGJSON_RESET ", 14) == 0) {
        ...
        fflush(stdout);
        int rc = apply_config_from_json(json, true);
    ...
    if (strncmp(line, "CFGJSON_FILE ", 13) == 0) {
        ...
        json_buf = malloc(fsize + 1);
        if (!json_buf) {
            fclose(fp);
            (void)write(g_client_fd, "ERR alloc\n", 10);
            return 0;
        }
        read_bytes = fread(json_buf, 1, fsize, fp);
        fclose(fp);
        ...
        fflush(stdout);
        rc = apply_config_from_json(json_buf, false);
        free(json_buf);
    ...
    if (strncmp(line, "CFGJSON_FILE_RESET ", 19) == 0) {
        ...
        fflush(stdout);
        rc = apply_config_from_json(json_buf, true);
        free(json_buf);
...
static int handle_client_objectlist(json_t *root)
{
    ...
    include_properties_obj = json_object_get(root, "includeProperties");
    
    /* Vérifier si on doit inclure les propriétés */
    if (include_properties_obj && json_is_boolean(include_properties_obj)) {
        include_properties = json_boolean_value(include_properties_obj);
        printf("[CLIENT] includeProperties = %s\n", include_properties ? "true" : "false");
        fflush(stdout);
    }
    ...
    /* ═══════════════════════════════════════════════════════════════
     * Attendre la réponse OBJECT_LIST
     * ═══════════════════════════════════════════════════════════════ */
    
    printf("[CLIENT] Waiting for OBJECT_LIST response (timeout=60s)...\n");
    fflush(stdout);
    ...
    for (timeout = 0; timeout < 600; timeout++) {
        ...
        if (req->completed) {
            printf("[CLIENT] ✓ OBJECT_LIST response received after %.1fs\n", timeout * 0.1);
            fflush(stdout);
            break;
        }
        ...
        if (timeout > 0 && timeout % 50 == 0) {
            printf("[CLIENT] Still waiting for OBJECT_LIST... (%.1fs / 60s)\n", timeout * 0.1);
            fflush(stdout);
        }
    }
    ...
    /* Vérifier timeout */
    if (timeout >= 600) {
        printf("[CLIENT] ✗ Timeout waiting for OBJECT_LIST after 60 seconds\n");
        fflush(stdout);
        
        pthread_mutex_lock(&pending_mutex);
        if (req->completed && req->response_json) {
            printf("[CLIENT] ⚠️  Response arrived JUST before timeout\n");
            fflush(stdout);
        ...
        response = client_create_error_response("Timeout waiting for OBJECT_LIST (60s)");
        write(g_client_fd, response, strlen(response));
        write(g_client_fd, "\n", 1);
        free(response);
        return 0;
    ...
    /* ═══════════════════════════════════════════════════════════════
     * ÉTAPE 2: LIRE LES PROPRIÉTÉS SI DEMANDÉ
     * ═══════════════════════════════════════════════════════════════ */
    
    if (include_properties && req->response_json) {
        printf("[CLIENT] ═══════════════════════════════════════════════════════\n");
        printf("[CLIENT] ÉTAPE 2: Reading properties for all objects...\n");
        printf("[CLIENT] ═══════════════════════════════════════════════════════\n");
        fflush(stdout);
        ...
        if (!response_root) {
            ...
            if (objects_array && json_is_array(objects_array)) {
                ...
                for (i = 0; i < obj_count; i++) {
                    ...
                    properties_to_read[prop_count++] = PROP_DESCRIPTION;
                    ...
                    switch (obj_type) {
                        ...
                        case OBJECT_ANALOG_OUTPUT:
                        case OBJECT_ANALOG_VALUE:
                            properties_to_read[prop_count++] = PROP_PRESENT_VALUE;
                            properties_to_read[prop_count++] = PROP_UNITS;
                            properties_to_read[prop_count++] = PROP_OUT_OF_SERVICE;
                            properties_to_read[prop_count++] = PROP_STATUS_FLAGS;
                            break;
                        ...
                        case OBJECT_BINARY_VALUE:
                            ...
                            properties_to_read[prop_count++] = PROP_OUT_OF_SERVICE;
                            properties_to_read[prop_count++] = PROP_STATUS_FLAGS;
                            properties_to_read[prop_count++] = PROP_INACTIVE_TEXT;
                            properties_to_read[prop_count++] = PROP_ACTIVE_TEXT;
                            break;
                        ...
                        case OBJECT_MULTI_STATE_VALUE:
                            ...
                            properties_to_read[prop_count++] = PROP_OUT_OF_SERVICE;
                            properties_to_read[prop_count++] = PROP_STATUS_FLAGS;
                            properties_to_read[prop_count++] = PROP_STATE_TEXT;
                            break;
                            
                        case OBJECT_DEVICE:
                            /* Pour device, on lit juste le nom */
                            break;
                    ...
                    for (j = 0; j < (size_t)prop_count; j++) {
                        ...
                        if (!prop_req) {
                            printf("[CLIENT]   ✗ No free slot for property %s\n", prop_name);
                            fflush(stdout);
                            continue;
                        }
                        ...
                        if (sent_prop_invoke == 0) {
                            printf("[CLIENT]   ✗ Failed to send request for property %s\n", prop_name);
                            fflush(stdout);
                            pthread_mutex_lock(&pending_mutex);
                        ...
                        if (prop_received && prop_req->response_json) {
                            ...
                            if (prop_response) {
                                ...
                                if (unit_obj) {
                                    json_object_set(prop_data, "unit", unit_obj);
                                }
                                if (datatype_obj) {
                                    json_object_set(prop_data, "datatype", datatype_obj);
                                }
                                
                                /* Ajouter à l'objet properties */
                                json_object_set_new(properties_obj, prop_name, prop_data);
                                
                                printf("[CLIENT]   ✓ %s = ", prop_name);
                                if (json_is_string(value_obj)) {
                                    printf("\"%s\"\n", json_string_value(value_obj));
                                } else if (json_is_number(value_obj)) {
                                    printf("%.2f", json_number_value(value_obj));
                                    if (unit_obj && json_is_string(unit_obj)) {
                                        printf(" %s", json_string_value(unit_obj));
                                    }
                                    printf("\n");
                                } else if (json_is_boolean(value_obj)) {
                                    printf("%s\n", json_boolean_value(value_obj) ? "true" : "false");
                                } else {
                                    printf("(value)\n");
                                }
                                fflush(stdout);
                    ...
                    /* Ajouter l'objet properties à l'objet */
            ...
            if (enriched_response) {
                ...
                free(req->response_json);
                req->response_json = enriched_response;
            }
    ...
    if (req->response_json) {
        printf("[CLIENT] ✓ Sending final response (%zu bytes)\n", strlen(req->response_json));
        fflush(stdout);
        write(g_client_fd, req->response_json, strlen(req->response_json));
        write(g_client_fd, "\n", 1);
        free(req->response_json);
...
static int handle_client_readprop(json_t *root)
{
    ...
    if (!cmd_obj || strcmp(json_string_value(cmd_obj), "readprop") != 0) {
        return -1;  /* Not a readprop command */
    }
    
    /* Parse parameters */
    device_obj = json_object_get(root, "device");
    ip_obj = json_object_get(root, "ip");
    object_obj = json_object_get(root, "object");
    property_obj = json_object_get(root, "property");
    ...
    if (!object_obj || !json_is_string(object_obj)) {
        response = client_create_error_response("Missing object parameter");
        write(g_client_fd, response, strlen(response));
        write(g_client_fd, "\n", 1);
        free(response);
        return 0;
    ...
    if (!property_obj || !json_is_string(property_obj)) {
        response = client_create_error_response("Missing property parameter");
        write(g_client_fd, response, strlen(response));
        write(g_client_fd, "\n", 1);
        free(response);
        return 0;
    ...
    target_device_id = json_integer_value(device_obj);
    obj_str = json_string_value(object_obj);
    prop_str = json_string_value(property_obj);
    ...
    if (strcasecmp(prop_str, "present-value") == 0) {
        prop_id = PROP_PRESENT_VALUE;
    } else if (strcasecmp(prop_str, "object-name") == 0) {
        prop_id = PROP_OBJECT_NAME;
    } else if (strcasecmp(prop_str, "description") == 0) {
        prop_id = PROP_DESCRIPTION;
    } else if (strcasecmp(prop_str, "status-flags") == 0) {
        prop_id = PROP_STATUS_FLAGS;
    } else if (strcasecmp(prop_str, "out-of-service") == 0) {
        prop_id = PROP_OUT_OF_SERVICE;
    } else if (strcasecmp(prop_str, "units") == 0) {
        prop_id = PROP_UNITS;
    } else {
    ...
    if (prop_id >= MAX_BACNET_PROPERTY_ID) {
        response = client_create_error_response("Invalid property name");
        write(g_client_fd, response, strlen(response));
        write(g_client_fd, "\n", 1);
        free(response);
        return 0;
    ...
    printf("[CLIENT] Sending ReadProperty: %s[%u].%s to device %u\n",
           bactext_object_type_name(obj_type), obj_instance, prop_str, target_device_id);
    fflush(stdout);
...
static int handle_client_writeprop(json_t *root)
{
    ...
    object_obj = json_object_get(root, "object");
    property_obj = json_object_get(root, "property");
    value_obj = json_object_get(root, "value");
    datatype_obj = json_object_get(root, "datatype");
    priority_obj = json_object_get(root, "priority");
    ...
    if (!value_obj) {
        response = client_create_error_response("Missing value parameter");
        write(g_client_fd, response, strlen(response));
        write(g_client_fd, "\n", 1);
        free(response);
        return 0;
    ...
    if (priority_obj && json_is_integer(priority_obj)) {
        int prio_val = json_integer_value(priority_obj);
        if (prio_val >= 1 && prio_val <= 16) {
            priority = (uint8_t)prio_val;
        }
    }
    ...
    if (datatype_obj && json_is_string(datatype_obj)) {
        ...
        if (json_is_real(value_obj)) {
            ...
            datatype_str = "unsigned";
        } else if (json_is_boolean(value_obj)) {
            datatype_str = "boolean";
    ...
    if (strcasecmp(datatype_str, "real") == 0) {
        app_value.tag = BACNET_APPLICATION_TAG_REAL;
        if (json_is_string(value_obj)) {
        ...
        response = client_create_error_response("Unknown datatype");
        write(g_client_fd, response, strlen(response));
        write(g_client_fd, "\n", 1);
        free(response);
        return 0;
...
int main(int argc, char *argv[])
{
    ...
    printf("Registering client handlers for Who-Is/I-Am and remote read/write...\n");
    apdu_set_unconfirmed_handler(SERVICE_UNCONFIRMED_I_AM, client_i_am_handler);
    apdu_set_confirmed_ack_handler(SERVICE_CONFIRMED_READ_PROPERTY, client_read_property_ack_handler);
    apdu_set_confirmed_ack_handler(SERVICE_CONFIRMED_READ_PROP_MULTIPLE, client_read_property_multiple_ack_handler);
    apdu_set_confirmed_ack_handler(SERVICE_CONFIRMED_READ_RANGE, client_read_range_ack_handler);
    apdu_set_confirmed_simple_ack_handler(SERVICE_CONFIRMED_WRITE_PROPERTY, client_write_property_ack_handler);
    apdu_set_unconfirmed_handler(SERVICE_UNCONFIRMED_COV_NOTIFICATION, client_cov_notification_handler);
